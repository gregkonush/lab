...
class HelloServerWorkload extends Chart {
  constructor(
    scope: Construct,
    id: string,
...
function synthesize(): void {
  ...
  const baseConfig: HelloServerConfig = {
    name: "hello-server",
    namespace: "hello-server",
    image: "ghcr.io/gregkonush/hello-server:main",
    containerPort: 8080,
    servicePort: 80,
    replicas: 1,
    resources: {
  ...
  const devOverlayConfig: HelloServerConfig = {
    ...baseConfig,
  ...
  new HelloServerWorkload(baseApp, "workload", baseConfig);

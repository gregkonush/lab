...
const serviceName =
  process.env.OTEL_SERVICE_NAME ?? process.env.LGTM_SERVICE_NAME ?? "bonjour";
const serviceNamespace =
  process.env.OTEL_SERVICE_NAMESPACE ?? process.env.POD_NAMESPACE ?? "default";
...
const metricReader = new PeriodicExportingMetricReader({
  ...
  exportIntervalMillis: Number.isFinite(exportInterval)
    ? Math.max(exportInterval, 5000)
    : 15000,
});

...
export function PodDetailsComponent() {
  ...
  const { data: pod, isLoading, error } = useQuery(trpc.pod.byName.queryOptions({ podName }))
      ...
      <div className="container mx-auto py-6 px-4">
        ...
        <div className="border border-zinc-700 text-sm overflow-hidden rounded-md h-[calc(100vh-10rem)]">
          <div className="w-full py-2 text-sm text-red-400 flex items-center justify-center h-full">
            Error loading pod details: {error instanceof Error ? error.message : 'Unknown error'}
          </div>
        </div>
      </div>
    )
  ...
  if (!pod) {
    return (
      <div className="container mx-auto py-6 px-4">
        ...
        <div className="border border-zinc-700 text-sm overflow-hidden rounded-md h-[calc(100vh-10rem)]">
          <div className="w-full py-2 text-sm text-zinc-300 flex items-center justify-center h-full">Pod not found</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-zinc-100">{pod.metadata?.name || 'Pod Details'}</h1>
      </div>
